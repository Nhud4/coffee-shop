import React from 'react';
import { BarChartHome } from '@/component/chart/BarChart';
import NavigationDashboard from '@/component/navigasi/NavigasiDashboard';
import CardSummary from '@/component/card/CardSummary';
import { PieChartHome } from '@/component/chart/PieChart';
import TableCustomer from '@/component/table/TablePelanggan';

export default function Dashboard(){
  return (
    <div>
      <NavigationDashboard page={'dashboard'}/>
      <div className="w-full p-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-4">
          <CardSummary
            title="Jumlah Ulasan"
            label="20 Ulasan"
          />
          <CardSummary
            title="Julmah Ulasan Puas"
            label="20 Ulasan"
          />
          <CardSummary
            title="Jumlah Ulasan Tidak Puas"
            label="20 Ulasan"
          />
          <CardSummary
            title="Vocher Digunakan"
            label="10"
            totalData="20"
          />
        </div>

        <div className="mt-4 grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="col-span-3">
            <BarChartHome />
          </div>

          <PieChartHome />
        </div>

        <div className="mt-6 bg-white p-4 rounded-lg shadow-1">
          <h1 className="text-3xl font-bold text-cyn-20 mb-4">Detail Kepuasan Pelanggan</h1>

          <TableCustomer />
        </div>
      </div>
    </div>
  );
}
